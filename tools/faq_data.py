...
"privacy-and-confidentiality": [
 ("What is the difference between anonymous, confidential, and pseudonymized data?",
  "They are three different promises. <strong>Anonymous</strong> data were never linked to identities, or every link has been irreversibly destroyed — no one, not even you, can trace a response back to a person, which is why anonymous data can be shared freely. <strong>Confidential</strong> data <em>are</em> identifiable (you know who said what) but you undertake to protect that link and never disclose individual responses; most interview and clinical studies are confidential, not anonymous. <strong>Pseudonymized</strong> data replace direct identifiers with a code while a separate key still links codes to people, so they are reversible by design and not anonymous while the key exists. The practical rule: if you could still identify anyone, the honest word on your consent form is <em>confidential</em>, not anonymous."),
...
"plagiarism-authorship-and-citation": [
 ("Can I reuse text from my own earlier assignment?",
  "Not by silently pasting it in — that is self-plagiarism. Reusing your own previously submitted or published work as though it were new misrepresents it, and most institutions treat re-submitting an assessed essay as an academic-integrity issue. If the earlier work was graded or published you should cite it, and you should rework the material for its new context rather than dropping identical paragraphs into a thesis; swapping a few words to disguise the reuse (patchwriting) does not help. Building on your own earlier writing is normal and fine — the honest way to do it is to disclose it, cite it, and rewrite. If you are unsure of your programme's specific rules, ask your supervisor before you reuse anything."),
 ...
 ("Do I have to read every paper I cite?",
  "In effect, yes — you should cite what you have actually read. If you rely on a claim, read the source it comes from; citing a paper you have only seen quoted elsewhere, as though you read the original, is <strong>citation laundering</strong>, because you vouch for a source you never checked and can silently pass on its errors. If you genuinely cannot access an original but need to mention it, cite it honestly as a secondary source — for example '(Smith, 1998, as cited in Jones, 2020)' — which makes clear what you actually consulted. You need not read every paper cover to cover, but you should at least read the part that supports the specific point you attribute to it; abstracts can misstate a study's nuance or even the direction of its results."),
],

"ai-in-research-ethics": [
 ("Can I use ChatGPT or Claude to write my thesis?",
  "Not to write it for you and hand it in as your own — that misrepresents whose work it is, and most universities treat submitting AI-generated text as your own as an academic-integrity breach. What is widely accepted is using AI as an assistant on work you <em>own and understand</em>: explaining an error, critiquing your argument, brainstorming, or polishing the grammar of paragraphs you wrote. The dividing line is authorship and verification: you must be able to stand behind every claim, number, and citation as if you had produced it, because you are the one accountable for it. And disclosure rules vary by institution and journal, so check your specific policy and declare AI use honestly when it asks you to."),
 ("Why does AI invent references that don't exist?",
  "A large language model generates text by predicting plausible next words, not by looking up a database of real papers. A reference is just a pattern — author, year, journal, title — so the model can produce one that looks completely convincing yet corresponds to no real paper: right-sounding authors, a real journal, a plausible title, all assembled from nothing. This is often called <strong>hallucination</strong>, and it is a structural feature of how the tool works, not an occasional bug. The practical rule is to treat every AI-suggested reference as unverified: find the actual paper, confirm it exists and says what you're citing it for, and cite that — never paste an AI reference list into your work unchecked."),
...
"fraud-and-self-correction": [
 ("What is the GRIM test?",
  "GRIM stands for <strong>Granularity-Related Inconsistency of Means</strong>. It's a simple arithmetic check for means built from whole-number responses — a count, or a single Likert-style item. Because a mean is the total divided by the number of people, and the total of <em>N</em> integers is itself an integer, the only means that can actually occur are fractions with denominator <em>N</em>: 0/<em>N</em>, 1/<em>N</em>, 2/<em>N</em> and so on. So a reported mean is only possible if some whole-number total, divided by <em>N</em> and rounded to the stated decimals, reproduces it. When it doesn't — say a mean of 5.19 from 28 people, where the nearest reachable values are 5.18 and 5.21 — the number cannot have come from that many integer responses, signalling a typo, a miscalculation, or fabricated data. GRIM only bites when <em>N</em> is smaller than 10 to the power of the decimals reported (e.g. under 100 for two decimals); with larger samples the steps are finer than the rounding and every mean becomes reachable."),
 ...
 ("How common is research fraud?",
  "Outright fabrication or falsification is rare. In anonymous surveys pooled by Fanelli (2009), about <strong>2%</strong> of scientists admitted to having fabricated, falsified, or modified data at least once, and higher fractions said they had seen colleagues do it. The far more common problem is the grey zone of <a href=\"../../ethics/questionable-research-practices/\">questionable research practices</a> — selective reporting, optional stopping, HARKing — which many more researchers admit to and which do more cumulative damage to the literature than the rare fraud case. The reassuring framing is that fraud is uncommon, that most researchers are honest, and that the fixes are structural: better incentives, transparency, open data, and detection tools mean fabricated results are increasingly likely to be caught."),
],
...
"dimensionality-reduction": [
 ...
 ("Can I run statistics on t-SNE or UMAP coordinates?",
  "No. A t-SNE or UMAP embedding is built to preserve <em>local</em> neighbourhoods for the purpose of visualization, and in doing so it deliberately distorts global geometry: distances between clusters, the sizes of clusters, and even the number of apparent clumps all change with the perplexity or <code>n_neighbors</code> setting and with the random seed. So the axes have no units, the gaps mean nothing, and running a t-test, a correlation, or a clustering on the 2-D coordinates produces numbers that describe the algorithm, not your data. Use the embedding only to spot structure worth investigating, then go back to the original feature space (or a <a href=\"../../stats-3/factor-analysis-pca/\">PCA</a>) to measure anything."),
 ("How many principal components should I keep?",
  "There is no universal answer — it depends on your goal — but a few standard aids help. Plot a scree graph (variance explained per component) and look for the 'elbow' where the curve levels off; keep components above the bend. Or keep enough components to reach a target cumulative variance, often 80–90%. The old Kaiser rule (keep every component with an eigenvalue above 1, i.e. above the average) is popular but tends to over-retain, so treat it as a loose upper bound rather than a law. If PCA is a preprocessing step for a predictive model, the cleanest approach is to let cross-validation choose the number of components by held-out performance."),
],
