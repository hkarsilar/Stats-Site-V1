...
## Phase 17 — Teaching alongside Stats 1 (P79–P87)

Added 6 Sep 2026 from a reading of the 2026/27 Stats 1 lecture decks and syllabus against the site (ROADMAP.md's Phase 17 addendum has the gap list, the week-by-week crosswalk, the rules for the phase and the running order with dates). The course follows Moore, McCabe & Craig chapters 1–9, examines on paper with printed tables, a formula sheet and a basic calculator, and already sends students to named site pages every week. The gaps are all one shape: the site teaches the software dialect where the course examines the textbook dialect (quartiles, two-sample df, σ-known z procedures, paper tables), plus one whole lecture (probability rules and Bayes) with no home, and an instructor surface that cannot carry lecture data. Model/effort legend as in Phase 12. **P79–P86 share registries — run one at a time, in the order the addendum gives. P87 last.** The site stays generic: nothing institution-specific, no slide text copied, the textbook's conventions labeled as "textbook", the site's own numbers throughout.

### P79 — Week 1: describing data the textbook way

**→ Powerful · Extra**

```
StatsCapybara roadmap prompt P79 (see ROADMAP.md — read the Phase 17 addendum first). Run node tools/audit.js first. Shares registries with P80–P86: run alone.

Week 1 of an intro course: describing a batch of numbers the way the textbook (Moore, McCabe & Craig) and a paper exam do it. Five items, all Stats 1.

1. Quartile conventions, labeled. descriptives.html interpolates R's type 7; the textbook takes the median of each half of the sorted data (the overall median excluded from both halves when n is odd); SPSS's Frequencies default is the weighted average (type 6). On the 18-value dataset in item 3 these give Q1/Q3 of 26.25 / 38.25, 26 / 39 and 26 / 40.75 — three answers to one exam question. Add a "Quartiles" segmented control to descriptives.html (Textbook · SPSS · R/Python) that changes Q1, Q3, IQR, the fences, the outlier flags and the boxplot; default stays R/Python (existing links, existing verified numbers). One short paragraph beside the control explains the three; one sentence in §1.3 and §1.4 says which convention the site's own pictures use. math-check.js assertions: the three (Q1, Q3) pairs above, plus the 13-value set 0.6, 1.2, 1.5, 1.6, 1.9, 2.1, 2.3, 2.5, 2.5, 2.8, 2.9, 3.3, 3.4 → textbook 1.55 / 2.85, type 7 1.6 / 2.8 — driven through the shipped page under the DOM shim, the way section 8 drives the dice lab.
2. §1.3 gains "Quartiles, the five-number summary and the 1.5 × IQR rule": the step-through the lesson's SD viz already does for spread, done for position (sort → median → halves → Q1 and Q3 → IQR → fences → flag), the same "Build it up" button pattern, textbook convention stated on the viz, the boxplot drawn last with the flagged point as a dot. The prose adds mean-vs-median-vs-mode for skew (mean above the median = right skew) with the worked numbers, and points at §1.4 for histograms and reading a boxplot. One new problem in problems.html (five-number summary, the rule, a boxplot sketch; textbook convention) with its verification comment.
3. Data-carrying URLs. Two surfaces learn ?data=<comma-separated numbers> (up to 200 values; garbage ignored per SC.preset's rules; documented in the script-top comment and in teachers.html's table): the §1.3 Mean vs. Median Playground (the values become the dots, replacing the seeded set, frozen from then on) and descriptives.html (prefills the textarea and runs). Ship one lecture-sized dataset on datasets.html as a card + CSV, hotdog-contest-2019.csv: the 18 finishing totals of the 2019 Nathan's Famous contest — 22, 22, 26, 26, 26, 27, 27, 28.5, 28.5, 29, 34, 34, 36, 39, 46, 47, 50, 71 (public contest results; cite the source on the card). Verified numbers for the card's HTML comment: mean 34.39, median 28.75, mode 26, textbook Q1 26 / Q3 39, IQR 13, fences 6.5 and 58.5, one outlier (71), sum of squares 2627.78, s² 154.58, s 12.43. Link the card from §1.3; put its ?data= URL in teachers.html.
4. SPSS for the first weeks. software.js has no entry for §1.2–§1.5 (12 of 18 Stats 1 lessons have none), and the first SPSS seminars cover exactly this. Add entries for types-of-data (Variable View: Measure = nominal / ordinal / scale, and why "scale" lumps interval with ratio), describing-data (Frequencies with Statistics → quartiles, mean, median, mode, SD; Explore for the five-number summary + boxplot, with its "weighted average" vs "Tukey's hinges" percentile note), visualizing-data (Chart Builder: histogram with a set bin width, bar chart, boxplot) and z-scores-and-the-normal-distribution (Descriptives → "Save standardized values as variables"). Each with the JASP equivalent, an APA sentence where one exists (descriptives), and tips. Rule 12 spelling; advice-terms.js and untaught-names.js clean.
5. Cross-links from §1.6 to methods/reliability-and-validity and methods/quasi-experiments: validity vs reliability and natural experiments both sit in the textbook's chapter 3, and the site teaches them a track away.

Verify: audit; math-check (descriptives math changed); prose-lint --strict; widget-terms, advice-terms, link-promises, destination-promises clean or ACKED with a reason; a ?data= URL carrying the 18 values reproduces every number above in both surfaces; a garbage ?data= loads the plain page; the subpath server. Rebuild the search index. Update teachers.html's preset table and CLAUDE.md's descriptives and preset passages. Tick P79 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P80 — Weeks 1–2: the normal curve on paper (Tables A, D, F)

**→ Powerful · Extra**

```
StatsCapybara roadmap prompt P80 (see ROADMAP.md, Phase 17 addendum). Run node tools/audit.js first. Run alone.

Intro exams are on paper: a standard normal table (Table A: area to the LEFT of z; rows z to one decimal, columns the second decimal), a t table (Table D: rows df, columns upper-tail p, confidence levels C along the foot, a z* row for df = ∞) and a χ² table (Table F: rows df, columns upper-tail p). tables.html is a calculator; nothing on the site teaches reading a printed table, and the calculator's fourth decimal differs from the table's (an exact .0455 where a student reading the table gets 2 × .0228 = .0456). Three items.

1. A "Printed tables" mode in tables.html beside the calculator (the page's own .seg pattern). Render Table A (z from −3.4 to 3.4, VIZ.normCdf to 4 dp), Table D (df 1–30, 40, 50, 60, 80, 100, 1000, ∞; upper-tail p .25 .20 .15 .10 .05 .025 .02 .01 .005 .0025 .001 .0005, with C = 50%…99.9% along the foot; VIZ.tInv) and Table F (df 1–30, 40, 50, 60, 80, 100; the same p columns; VIZ.chiSqInv) as real <table>s inside .hscroll, the row and column headers highlighting on hover and focus, the calculator's current statistic highlighted in the table, and a click on a cell setting the calculator. Print-friendly through the shared print block (one table per page). Verify against printed values: P(Z < −2.40) = .0082, P(Z < −2.41) = .0080, P(Z < 1.00) = .8413, P(Z < −2.33) = .0099, z* for 95% = 1.960, t*(df 5, 95%) = 2.571, χ²(df 6, α .05) = 12.59, χ²(df 1, α .05) = 3.84. math-check.js: a grid of table cells asserted against the shipped page, driven under the DOM shim the way section 8 drives the dice lab.
2. A "Look it up" drill on the same page: a generator of table tasks (P(Z < z), P(Z > z), P(z₁ < Z < z₂), the z for a given left area, z* or t* for a confidence level and df, the χ² critical for a df and α); the student types the table's answer; the check accepts the table's 4-dp value, not only the exact one. Ten tasks, a tally, no timer, no storage. Above it, the three moves as a reminder: left = read; right = 1 − read, or the symmetry read of −z; between = larger area − smaller area.
3. §1.5 gets those three moves in prose with a worked example (heights N(164, 6): P(X < 170) → z = 1.00 → .8413, and its complement .1587), the density-curve vocabulary the textbook uses (area under the curve = proportion of observations; total area 1), and presets on the Z-Score Explorer: ?mu= ?sigma= ?x= and ?shade=left|right|between, documented atop the script and in teachers.html's table. The 68–95–99.7 viz stays as it is.

Verify: audit; math-check (new tables math and the page-driving block); prose-lint --strict; widget-terms clean; the three tables scroll inside themselves at 360px; the drill works keyboard-only; light + dark; the subpath server. Rebuild the search index. Tick P80 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P81 — Week 2: probability, the whole lecture

**→ Extra Powerful · Max effort**

```
StatsCapybara roadmap prompt P81 (see ROADMAP.md, Phase 17 addendum). Run node tools/audit.js first. Run alone.

An intro course spends a whole lecture on probability rules, conditional probability and Bayes, and the site gives that lecture three paragraphs (§1.7 has the disjoint-only rules, one conditional paragraph and a Bayes FAQ; §1.8 opens on the binomial's four conditions). Build it out inside the two existing lessons — no new lesson, no renumbering.

1. §1.7 probability-basics grows from three sections to a full lesson, in the textbook's order: random phenomena and long-run frequency (the opener and the LLN viz stay); sample space and events; the four rules (0–1, total 1, complement, addition for disjoint events); the GENERAL addition rule P(A or B) = P(A) + P(B) − P(A and B) with a two-circle picture; independence and the multiplication rule; conditional probability P(B | A) and the GENERAL multiplication rule P(A and B) = P(A)·P(B | A), with the 2 × 2 "who cycles to work" table already in problems.html as the running example; Bayes' rule with the law of total probability spelled out, not only as a formula. Three worked callouts, each verified with node -e and written in the site's voice with the site's framing: de Méré's two bets (at least one six in four throws = 1 − (5/6)⁴ = .5177 versus at least one double six in 24 throws = 1 − (35/36)²⁴ = .4914 — the complement and multiplication rules doing all the work); the birthday problem (23 people → .5073, and the 253 pairs that explain why intuition fails); the prosecutor's fallacy (P(fits the description | innocent) is not P(innocent | fits the description); the real cases behind the name — Sally Clark, Lucia de Berk — in one careful sentence each, no verdict on the people, a link to methods/the-replication-crisis where base-rate logic returns).
2. A second viz for §1.7: 🎮 The Base-Rate Tree — a natural-frequency tree (1,000 people → ill / healthy → test positive / negative) with sliders for prevalence, sensitivity and specificity, the four leaf counts drawn as proportional boxes, and P(ill | positive) computed both ways in the readout: from the counts, and by Bayes with the total-probability denominator written out. Defaults 1 in 1,000, .99, .99 → 9.0%; a second preset in the umbrella-and-rain shape (prior .20, .90, .10 → 69.2%). Presets ?prev= ?sens= ?spec=. Nothing random in it.
3. §1.8 binomial-distribution gains a section BEFORE its four conditions: random variables (discrete vs continuous), the probability distribution of a discrete random variable as a table, its mean μ = Σ x·p and variance σ² = Σ (x − μ)²·p, worked on the classroom experiment intro courses run — guessing eight four-option multiple-choice answers at random: P(0…4) = .1001, .2670, .3115, .2076, .0865, mean 2, SD 1.2247 — then the binomial shortcut np and √(np(1 − p)) as the payoff. Presets on Every Possible Count: ?n= ?p= ?k= (the experiment is ?n=8&p=0.25). A "Simulate a class" button draws 30 random guessers and overlays their empirical bars on the exact ones (seeded, frozen; "New class" reseeds).
4. Registries: glossary terms (sample space, event, complement, disjoint events, general addition rule, conditional probability, independence, Bayes' rule, law of total probability, random variable, expected value — check which exist first); quiz.html +3 stats-1 questions on this lecture; 3 new problems in problems.html (a general-addition-rule count; a Bayes screening question with the tree drawn; a discrete random variable's mean and SD) with verification comments; FAQ swaps only where an answer now repeats the prose (faq-audit.js REPRISE); checks.js questions revised so all three test the hardest points of the new material.

Verify: audit; prose-lint --strict (a big prose addition — mind rule 1's two clauses); faq-audit; widget-terms; symbol-names (name Σ where you use it); link-promises; every number above reproduced with node -e; presets land; the subpath server. Rebuild FAQs and the search index. Update teachers.html's preset table. Tick P81 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P82 — Weeks 3–4: inference when σ is known, tails, and power

**→ Extra Powerful · Max effort**

```
StatsCapybara roadmap prompt P82 (see ROADMAP.md, Phase 17 addendum). Run node tools/audit.js first. Run alone.

The textbook introduces inference with σ KNOWN (chapter 6: the z-interval and the one-sample z-test) before it introduces t, and intro exams test that form by hand. The site has no z-procedure anywhere: §1.11 goes straight to t*, and §1.12's P-Value Explorer takes a z as input and never derives one. Add the missing rung, then the four week-4 ideas that the lessons currently give zero prose to.

1. §1.11 confidence-intervals gains "When σ is known: the z-interval" — x̄ ± z*·σ/√n, z* = 1.645 / 1.960 / 2.576 for 90 / 95 / 99% (read off the foot of P80's printed Table D; link it), the margin of error and its three levers (C, σ, n), the cautions (an SRS; σ must actually be known; §1.13 for the honest case). A second, small viz: 🎮 Interval Builder — inputs x̄, σ, n and a C seg; draws the sampling distribution in the ORIGINAL units with the interval on a number line and prints the margin. Presets ?xbar= ?sigma= ?n= ?level=. Check: x̄ = 222, σ = 5, n = 4, 95% → 222 ± 4.90 = [217.1, 226.9].
2. §1.12 hypothesis-testing-logic gains a "From numbers to z" panel above the P-Value Explorer (μ₀, σ, n, x̄ → z, then the explorer takes over; the existing ?z= preset stays the way to land on a bare z), the textbook's five-step algorithm (hypotheses → α → statistic → p → conclusion), and one worked test in the site's voice (a packaging line: μ₀ = 227 g, σ = 5, n = 4, x̄ = 222 → z = −2.00, two-sided p = .0455 → reject at α = .05, keep at .01). Presets ?mu0= ?sigma= ?n= ?xbar= compose with the existing ?tail= and ?alpha=.
3. Three concepts with zero prose hits today, each a short section of §1.12: (a) one-tailed vs two-tailed chosen FROM THE QUESTION'S WORDING ("higher than" → one tail; "different from" / "incorrect" → two; decided before the data, never after seeing the sign) with the p-value rule for each (left: read; right: 1 − read; two: double); (b) statistical vs practical significance (a small p says there IS a difference, not a big one; link the effect-size converter); (c) absence of evidence is not evidence of absence (a "study finds no link" headline unpacked; link writing/nonsignificant-results).
4. β and power in Stats 1 terms. Replace the two-bullet "Two ways to be wrong" with a section and a viz: 🎮 Two Curves — the null sampling distribution and the distribution under a true mean μ₁, α shaded on the null, β on the alternative, power printed as 1 − β; sliders for μ₁, n, σ, α. Presets ?mu1= ?n= ?alpha=. A verified anchor for the prose (recompute it, then cite it): μ₀ = 22.0, σ = 0.5, n = 5, α = .05 two-sided, μ₁ = 22.6 → power = .765. Keep the Stats 3 power lesson as the going-deeper link.
5. formulas.html: add the rows an intro formula sheet carries and the site lacks — the z-interval, the one-sample z, the two-sample t SE √(s₁²/n₁ + s₂²/n₂) with the conservative-df note, the one-proportion SE and CI, the two-proportion pooled p̂, the binomial mean and SD, a discrete random variable's mean and variance, the general addition and multiplication rules — in the site's notation. Registries: glossary (z-interval, margin of error, one-tailed test, two-tailed test, practical significance), quiz +3, checks.js revisited, 2 new problems (a z-interval and its margin, then "what n for a margin of m"; a σ-known z-test with the tail chosen from the wording and the decision at two α levels) with verification comments.

Verify: audit; prose-lint --strict; math-check untouched unless viz.js changed; widget-terms, advice-terms, symbol-names, link-promises clean; every anchor recomputed with node -e; presets; the subpath server; light + dark + 360px. Rebuild FAQs and the search index; teachers.html's preset table. Tick P82 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P83 — Weeks 4–5: the t procedures the exam way

**→ Extra Powerful · Max effort**

```
StatsCapybara roadmap prompt P83 (see ROADMAP.md, Phase 17 addendum). Run node tools/audit.js first. Run alone.

Chapter 7 by hand: the t procedures the way an intro exam asks for them, with the software answer beside the hand answer and both labeled.

1. §1.13 one-sample-and-paired-t-tests: name s/√n as the standard error of the mean (SE) where the statistic is introduced; the one-sample t CONFIDENCE INTERVAL x̄ ± t*·s/√n with t* from Table D at df = n − 1 (link P80's printed table); one sourced paragraph on Gosset (Guinness, small samples, "Student") where the t-distribution is introduced — no folklore; the textbook's robustness rule of thumb as a callout (n < 15: data close to normal and no outliers; 15 to 40: mild skew acceptable, no outliers; over 40: fine even with strong skew) with a sentence on why the CLT is what makes the last line true; the matched-pairs section writes H₀: μ_difference = 0 out, with the direction of the difference stated. Presets on the t-vs-normal viz: ?df= and a ?t= marker that shades both tails and prints the p-value read the way Table D reads it ("between the .05 and .025 columns") beside the exact one.
2. §1.14 independent-samples-t-test: the two-sample SE √(s₁²/n₁ + s₂²/n₂) and the CI (x̄₁ − x̄₂) ± t*·SE; THREE degrees-of-freedom conventions in one small table — the conservative hand rule df = min(n₁ − 1, n₂ − 1), software's Welch–Satterthwaite (already taught in §2.2; link, do not re-teach), and the pooled n₁ + n₂ − 2 under equal variances — with a worked pair of numbers showing that the conservative rule errs toward a wider interval and a larger p, which is why an exam accepts it; the two-sample z (σ₁ and σ₂ known) in one sentence; the textbook's planning guidelines (equal n if you can; n₁ + n₂ ≥ 40 for the most skewed data). The Tug-of-War viz gains a "df" seg (Conservative · Welch · Pooled) that changes the critical t and p in the readout so the three verdicts can be compared live. Presets ?gap= ?sd= ?n= ?df=.
3. A second viz for §1.14: 🎮 Overlapping Intervals, Different Means — the textbook's "common mistake": two 95% CIs that overlap while the two-sample test rejects. Frozen two-group data with a gap slider; draws both CIs and the CI for the difference; the readout states the overlap, the test's p and the one-sentence reason (the difference's SE is not the sum of the two SEs). Verify a clean case with node -e and cite it in the prose.
4. software.js: the one-sample entry gains the CI in its steps (Explore and One-Sample T Test both print one; note that SPSS prints the CI of the difference from the test value); the two-sample entry gains "which df row of the Independent Samples Test table is which" (Levene → equal variances assumed / not assumed) and the tip that the exam's conservative df will not match SPSS's row. Registries: glossary (standard error of the mean, conservative degrees of freedom, pooled t-test — check which exist), quiz +3, checks.js revisited, 2 new problems (a one-sample t CI by hand from summary statistics; a two-sample t at the conservative df and the same test at the Welch df, asking which an exam wants and why they agree here) with verification comments.

Verify: audit; prose-lint --strict; widget-terms, advice-terms, prescription-terms, destination-promises clean; numbers recomputed; presets; the subpath server. Rebuild FAQs and the search index; teachers.html's preset table. Tick P83 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P84 — Week 6: proportions and chi-square by hand

**→ Extra Powerful · Max effort**

```
StatsCapybara roadmap prompt P84 (see ROADMAP.md, Phase 17 addendum). Run node tools/audit.js first. Run alone.

Chapters 8–9 the way an intro exam asks them: proportions with the textbook's conditions, and a chi-square worked cell by cell on a table bigger than 2 × 2.

1. §1.15 inference-for-proportions: state the conditions in the textbook's form beside the site's — the CI wants at least 15 successes and 15 failures (the site says 10; both rules are in use, say which text uses which and why the site's own calculator keeps the 10 rule), the test wants np₀ and n(1 − p₀) ≥ 10 (already there), and the population at least 10–20× the sample (missing) — plus the honest note that the plain interval under-covers (one sentence and a link if the lesson has no Wilson or plus-four remark). Presets on One Proportion, or Two: ?n= ?x= ?p0= and ?n1= ?x1= ?n2= ?x2=.
2. §1.16 chi-square-tests: grow the Contingency Table Explorer into 🎮 Build a Table — 2–4 rows × 2–4 columns of editable counts; each cell shows observed, expected (row total × column total ÷ grand total) and its component (O − E)²/E; the components sum visibly into χ², with df = (r − 1)(c − 1), the exact p, and a Table-F-style reading ("between the .005 and .0025 columns") beside it; a component heat-tint so the cell doing the damage is visible; the conditions printed live (all expected ≥ 1; no more than 20% below 5; for 2 × 2, all four ≥ 5). Keep the 2 × 2 treatment example as the default state so existing prose and problem references still hold. Presets ?t=<row-major counts, commas within a row, semicolons between rows> (e.g. ?t=45,20;25,40), so a lecturer can load a slide's table live. A verified anchor for the prose: a 3 × 4 table of your own checked with node -e (χ² to 2 dp, df 6, exact p), and confirm the shape of the textbook's example, χ² = 19.86 with df 6 → p = .0029.
3. Prose for §1.16: the "components first, then sum" working tip; the χ² family in one paragraph (squares of standard normals, df; link distributions.html); the conditions above; the 2 × 2 ↔ two-proportion identity χ² = z² pointing at §1.15's existing section rather than repeating it.
4. software.js: the chi-square entry gains "Cells → Expected, and Unstandardized residuals" so the SPSS table shows the same three numbers the viz shows; the proportions entry notes where SPSS 29 keeps its one-sample proportion test. Registries: glossary (expected count, chi-square component, conditional distribution), quiz +3, checks.js revisited, 2 new problems (expected counts and components on a 2 × 3 table; a two-proportion z with the pooled p̂) with verification comments; math-check.js: a section asserting the builder's χ² on the 2 × 2 default and on one r × c table, driven under the DOM shim if the builder's arithmetic lives in the page.

Verify: audit; math-check; prose-lint --strict; widget-terms and symbol-names clean; a 4 × 4 table scrolls inside itself at 360px; cells edit keyboard-only; presets; the subpath server. Rebuild FAQs and the search index; teachers.html's preset table. Tick P84 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P85 — Week 7: correlation and regression by hand

**→ Extra Powerful · Max effort**

```
StatsCapybara roadmap prompt P85 (see ROADMAP.md, Phase 17 addendum). Run node tools/audit.js first. Run alone.

Chapter 2 by hand: r as a formula a student can compute, the regression line from r and two SDs, and the two class exercises every intro lecture runs.

1. §1.17 correlation gains: the textbook definition r = (1/(n − 1)) Σ [(x − x̄)/sₓ]·[(y − ȳ)/sᵧ] built as a step-through table (📏 like §1.3's SD viz: deviations → standardized → products → mean) on a frozen five-point set of your own (verify r with node -e and print it in the prose); the "facts about r" in the textbook's shape (no units, symmetric in x and y, −1 to 1, linear only, outlier-sensitive — gather what the lesson already says); scatterplot vocabulary (form, direction, strength, then deviations from the pattern) and the scaling warning (a roughly square plot with no dead space); and 🎮 Guess the r — ten frozen scatterplots (seeded, the same ten every visit so a class can compare), the student types a guess, the readout gives the true r and the running mean absolute error; no timer, no storage, keyboard-first. Add ?data=x1:y1,x2:y2,… to the Correlation Explorer (up to 100 pairs; replaces the generated cloud with the given points, frozen) beside the existing ?r= ?line= ?ellipse=, so a lecturer can load lecture data from the URL without the site shipping it.
2. §1.18 simple-linear-regression gains the hand formulas in a section of their own: b₁ = r·sᵧ/sₓ, b₀ = ȳ − b₁x̄, "the line passes through (x̄, ȳ)", the prediction ŷ and a one-sentence extrapolation warning; r² as the share of variance explained, in the textbook's words; and a "Residual plot" toggle on the Least-Squares Playground (residual against x below the scatter, a zero line, the three patterns to look for: random, curved, fanning; link stats-2/regression-diagnostics for the full treatment). The same ?data= preset as §1.17, parsed by one shared helper so the two lessons cannot drift. The lurking-variable paragraph gains the "a categorical variable hides two positive trends inside one negative one" example (a small inline figure is enough).
3. Registries: glossary (residual plot, extrapolation, lurking variable — check which exist), quiz +3, checks.js revisited, 3 new problems (r by hand from a five-point table; the line from r, sₓ, sᵧ, x̄, ȳ and one prediction; reading a residual plot) with verification comments; software.js correlation and regression entries gain the "save residuals" step so the residual plot can be reproduced in SPSS.

Verify: audit; prose-lint --strict; widget-terms clean (Guess the r prints new labels — explain them in prose); link-promises; numbers recomputed; ?data= round-trips; presets; the subpath server; 360px. Rebuild FAQs and the search index; teachers.html's preset table. Tick P85 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P86 — Exam prep: section-range mock exams + exam-style problems

**→ Powerful · Extra**

```
StatsCapybara roadmap prompt P86 (see ROADMAP.md, Phase 17 addendum). Run node tools/audit.js first. Edits quiz.html and problems.html: run alone.

Intro courses examine in halves — a midterm on the first weeks, a final on all of them — and the site's exam mode can only scope by whole course. Two items.

1. quiz.html: exam scope by SECTION RANGE. Beside each course's checkbox, a "from §a to §b" control populated from CURRICULUM (default = all), so "Stats 1, §1.1–§1.12" is one exam. The sc-exam signature becomes e.g. "stats-1:1.1-1.12"; keep "all" and the plain course-slug forms so stored bests survive (no migration). The end screen's per-course breakdown shows the range. A shareable URL, quiz.html?exam=stats-1&from=1.1&to=1.12&n=20, opens exam mode with that scope and length preselected (the exam still starts on the button; garbage params ignored, SC.preset's rules), and teachers.html gets a copy-ready example under a short "Set a mock exam" heading. The BANK needs a tag a range can read: every question already carries c; add an optional s (the section number as a string, "1.12") to every Stats 1 and Stats 2 question at least (look each one up; a question spanning two lessons takes the later one), and let range scope fall back to the whole course for a course with no s tags, with the scope line saying so. exam_finished's scope param carries the new signature — same event, same param, so privacy.html is unchanged.
2. problems.html: a second Stats 1 set, "Exam-style", 8 problems in the shapes intro exams ask and the current 12 do not: (1) five-number summary and the 1.5 × IQR rule, textbook quartiles, from 13 raw values; (2) Table-A reading, three probabilities under N(μ, σ) including a "between"; (3) probability rules — a general-addition-rule question and a de Méré-shaped complement question; (4) Bayes with a natural-frequency tree; (5) a discrete random variable's mean and SD from a probability table; (6) binomial: n choose k by hand, P(X = k) and P(X ≤ 1); (7) a z-interval with its margin, then "what n for a margin of m"; (8) a σ-known z-test with the tail chosen from the wording, the decision at two α levels, and the Type I / Type II sentence for that decision. Every number verified (node -e with VIZ; binomial via exact integers), the one-liners in the HTML comment, difficulty chips, .pb-ref lines to §1.3, §1.5, §1.7, §1.8, §1.11, §1.12 and to the tools that check them (descriptives.html with ?data=, tables.html). The problems are the site's own: the same shapes as a textbook's, different numbers and stories. Keep the numbering continuous and the page's em-dash count where it is.

Verify: audit (fragment ids resolve); prose-lint --strict; the exam flow end to end (range scope, the shareable URL, a retry run not recorded, the sc-exam signature); keyboard; the subpath server. Rebuild the search index. Tick P86 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P87 — The instructor surface: block map, present mode, preset sweep

**→ Powerful · Extra**

```
StatsCapybara roadmap prompt P87 (see ROADMAP.md, Phase 17 addendum). Run node tools/audit.js first. Run AFTER P79–P86: it documents their presets.

The instructor surface, made to match how intro statistics is actually scheduled and taught in a room.

1. teachers.html: beside the existing 14-week Stats 1–2 map, a 7-week BLOCK map for a stand-alone Stats 1 (week → topics → lessons → tools → the problems set), in the order the textbook teaches: looking at data and producing data; probability; sampling distributions and the start of inference; inference part two and inference for means; means continued; proportions and categorical data; correlation and regression; then a mock exam and the exam. Each cell links the lesson with the preset a lecturer would put on the slide (?data= for §1.3, ?mu=&sigma= for §1.5, ?prev= for §1.7, ?n=8&p=0.25 for §1.8, ?mu0=… for §1.12, ?t= for §1.16). The same .sem-map / .ref-table / .hscroll pattern; prints on one page.
2. Present mode: ?present=1 on a lesson = embed mode plus a projector scale (larger type on the viz's readouts and controls, the PNG button hidden, the back-link kept) — for the room, not the iframe; composes with presets; documented in teachers.html beside ?embed=1 with one copy-ready example. site.js and styles.css are precached: bump CACHE_VERSION.
3. Completeness sweep: teachers.html's preset table lists every lesson that calls SC.preset after P79–P86 (write a five-line node check comparing the table's slugs against grep -l "SC.preset"; run it; paste the result under the tick); CLAUDE.md's preset passage lists the new params; the "Adding a lesson" checklist mentions ?data= as an option for data-driven vizzes.

Verify: audit; prose-lint --strict; light + dark + 360px; the block map prints on one page (the P30 CSSOM flip); present mode in a real window at 1280 × 720; the subpath server. Tick P87 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

## Phase 18 — Teaching alongside Stats 2 (P88–P96)

Added 8 Sep 2026 from a reading of the Stats 2 lecture decks and syllabus against the site (ROADMAP.md's Phase 18 addendum has the gap list, the crosswalk, the running order and the slide notes). The course examines on paper with a formula sheet and a basic calculator, and every week of it fills in an ANOVA table by hand; the site teaches every idea in the course and builds that table nowhere. Phase 17's rules carry over (the site stays generic; both dialects labeled, no default silently switched; exam-first; presets carry lectures; the iron rules), plus two of its own: **the table is the interface** (hand arithmetic lives in a cell-by-cell ANOVA table, and "complete the missing cells" is a mode of that table), and **group data in a URL is `?g=`** (commas inside a group, semicolons between groups, one shared parser beside `SC.dataParam` and `SC.pairParam`). One fact every ANOVA prompt needs: **`VIZ.sd` divides by n**; every exam-dialect s, MSE or sₚ needs the n − 1 form. Model/effort legend as in Phase 12. **P88–P95 share registries — run one at a time, in the addendum's order. P96 last.**

### P88 — Week 1: non-parametric tests by hand and by table

**→ Extra Powerful · Max effort**

```
StatsCapybara roadmap prompt P88 (see ROADMAP.md — read the Phase 18 addendum first). Run node tools/audit.js first. Shares registries with P89–P95: run alone.

An intermediate course opens with the non-parametric tests done by hand against critical-value tables, on paper. §2.1 teaches the logic of U and W well and prints exact p-values; it never shows the arithmetic, has no tables, and gives Spearman one line. Add the hand dialect beside the exact one, label both.

1. §2.1 non-parametric-alternatives: the textbook vocabulary up front (parametric = assumes a distribution and estimates its parameters; non-parametric = distribution-free; the equivalence table — independent t → Mann–Whitney U / Wilcoxon rank-sum, paired t → Wilcoxon signed-ranks, Pearson → Spearman, one-way ANOVA → Kruskal–Wallis, RM-ANOVA → Friedman) and the common recipe (pool, rank with average ranks for ties, sum, look up). Then the arithmetic each test's section is missing: U₁ = n₁n₂ + n₁(n₁ + 1)/2 − R₁ and U₂ likewise, U₁ + U₂ = n₁n₂, the test statistic is the smaller U and the table says reject when U ≤ U*; W from the signed ranks (discard zero differences, rank |D|, T₊ and T₋, W = the smaller, reject when W ≤ W*); and a new Spearman section — rank both variables, d = difference of ranks, ρₛ = 1 − 6Σd²/(n(n² − 1)) (state that the shortcut is exact only without ties, and that with ties it is Pearson on the ranks, which is what §1.17 already says), tested against its own table. One-tailed tests by doubling α, and the honest cost: less power than the parametric test when that test's assumptions hold. Keep the exact p-values; add the table reading beside each ("U = 37 against U* = 14: keep H₀").
2. The rank ledger viz gains a "by hand" readout panel for its Mann–Whitney and Wilcoxon tabs (the rank sums, both U's or T₊/T₋, the statistic, the critical value at the chosen α and tails, the verdict) and a Spearman tab (two ranked columns, the d² column, ρₛ). Presets ?tab= exists; add ?g= for the two-group tabs (commas in a group, semicolons between) and ?data=x:y pairs (SC.pairParam) for Spearman.
3. tables.html "Printed tables" gains three more: Mann–Whitney U critical values (n₁, n₂ from 3 to 20; two-tailed .05 and .01, one-tailed .05), Wilcoxon signed-ranks W (n from 5 to 30; the same columns) and Spearman ρₛ (n from 5 to 30). Compute them EXACTLY from the null distributions — U and W by dynamic programming over rank sums, ρₛ by enumeration up to the n where that is feasible and the t-approximation beyond, with the switch stated in the table's note — never by transcribing a printed table. math-check.js: assert a grid of cells against a published table (Siegel & Castellan or the open chapter the course uses), e.g. U*(8, 8; two-tailed .05) = 13, W*(10; two-tailed .05) = 8, ρₛ*(9; two-tailed .05) = .683 exact; where a printed table and the exact value disagree by a rounding convention, keep the exact value and say so in the note. Drive the shipped page under the DOM shim the way section 8 drives the dice lab. The look-it-up drill gains U/W/ρₛ tasks.
4. Registries: glossary (rank sum, U statistic, signed rank, Spearman's rho — check what exists), quiz +3 stats-2 questions tagged s: "2.1", checks.js revisited, 3 new problems (U by hand with the table; W by hand; ρₛ by hand) with verification comments (exact null distributions in the comment, as P63 did for the Mann–Whitney enumeration); software.js: where SPSS prints U, W and the exact vs asymptotic p (Legacy Dialogs vs the Nonparametric Tests menu), and that SPSS reports the smaller U.

Verify: audit; math-check (new tables + the page-driving block); prose-lint --strict; widget-terms (new labels — explain them), advice-terms, symbol-names (ρ is named; check ρₛ) clean; the tables scroll inside themselves at 360px; presets; the subpath server. Rebuild FAQs and the search index; teachers.html's preset table. Tick P88 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P89 — Weeks 1–2: the ANOVA table by hand

**→ Extra Powerful · Max effort**

```
StatsCapybara roadmap prompt P89 (see ROADMAP.md, Phase 18 addendum). Run node tools/audit.js first. Run alone.

Every week of an intermediate course fills in an ANOVA table by hand, and the site builds that table nowhere: §2.2's viz prints two mean squares and the prose never writes a sum of squares. This prompt makes the table the interface, and it is the pattern P90–P92 extend.

1. §2.2 one-way-anova gains, in the textbook's order: explained vs unexplained variance (signal and noise, the two "which mean?" questions — the grand mean and the group means); the sums of squares written out, SSG = Σnᵢ(x̄ᵢ − x̄)², SSE = Σ(nᵢ − 1)sᵢ² = Σ(x − x̄ᵢ)², SST = Σ(x − x̄)² = SSG + SSE; the df I − 1, N − I, N − 1; MS = SS/df; F = MSG/MSE on F(I − 1, N − I); sₚ = √MSE as the pooled SD every follow-up test reuses; η² = SSG/SST from the same table; the model view (data = fit + residual, each score = its group mean + its residual); the error-control reason (k groups → k(k − 1)/2 pairs; 1 − (1 − .05)³ = .1426) as the honest arithmetic behind the "why not lots of t-tests" section; and F = t² when there are two groups, verified on the site's own numbers.
2. A second viz: 🎮 Build the ANOVA Table — a small editable dataset (3–4 groups × 4–8 values, a frozen default), a "Build it up" step-through in the §1.3 pattern (group means → deviations from the grand mean → SSG → within-group deviations → SSE → SST → df → MS → F → p → sₚ and η²), and the table itself drawn cell by cell as the steps run. An "Exam mode" toggle blanks a random set of cells (an SS, a df, an MS, the F) for the student to fill in from the others, with a check; that is the item an ANOVA exam asks, so it lives in the same table. Presets ?g=<groups: commas inside, semicolons between> (the shared parser this prompt adds beside SC.dataParam and SC.pairParam in site.js — document it in CLAUDE.md's preset passage) and ?step=. The existing F-Ratio viz stays as the intuition; this one is the arithmetic. IMPORTANT: VIZ.sd divides by n; use a sample-SD helper for every s, MSE and sₚ here, and add a math-check assertion that the table's MSE on the frozen default equals the n − 1 form.
3. software.js one-way-anova: reading SPSS's ANOVA table row by row (Between Groups / Within Groups / Total; where η² comes from; that SPSS's Descriptives box gives the per-group s the hand SSE needs), and the JASP equivalents. formulas.html: SSG, SSE, SST, the three df, MS, sₚ, the F ratio's distribution, η² — in the form a formula sheet prints them.
4. Registries: glossary (sum of squares, mean square, grand mean, pooled standard deviation — check what exists), quiz +3 stats-2 questions tagged s: "2.2", checks.js revisited, 2 new problems (from three groups' means, SDs and n to the full table and the verdict; a partly filled table to complete, with the F verdict and η²) with verification comments.

Verify: audit; math-check (the divisor assertion + the table driven under the DOM shim on the frozen default, cell by cell); prose-lint --strict; widget-terms, symbol-names (η is named) clean; ?g= round-trips; exam mode keyboard-only; the subpath server; 360px. Rebuild FAQs and the search index; teachers.html's preset table. Tick P89 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P90 — Week 2: planned contrasts and multiple comparisons by hand

**→ Extra Powerful · Max effort**

```
StatsCapybara roadmap prompt P90 (see ROADMAP.md, Phase 18 addendum). Requires P89 (the ?g= parser and the table pattern). Run node tools/audit.js first. Run alone.

After the omnibus F, an intermediate course does two things by hand that the site does not: planned contrasts, and pairwise comparisons with the pooled error term. §2.3 gives "planned vs. post-hoc" one paragraph and no arithmetic.

1. §2.3 post-hoc-tests gains a section on planned contrasts BEFORE the corrections: a contrast is a weighted combination of means, ψ = Σaᵢμᵢ, with coefficients that sum to zero (simple pairwise [1, −1, 0, 0]; complex, one group against the average of the rest [3, −1, −1, −1] or [1, −⅓, −⅓, −⅓] — the same contrast, and why); the sample contrast, SE_ψ = √(MSE·Σaᵢ²/nᵢ) with MSE from the ANOVA table, t = ψ/SE_ψ on df = N − I, the CI ψ ± t*·SE_ψ and the sentence that a CI excluding zero and a significant t are the same fact; one-tailed tests are legitimate here because the direction was planned; and the planned-vs-post-hoc contrast in the lecture's terms (few, directional, theory-driven, little correction — versus exploratory, two-tailed, all pairs, corrected).
2. Then the pairwise arithmetic the corrections act on: t = (x̄ᵢ − x̄ⱼ)/(sₚ√(1/nᵢ + 1/nⱼ)) with sₚ = √MSE and df = N − I; the Bonferroni per-test α/k and the critical t** it implies, beside the plain t* (a verified pair: with three comparisons at df = 73, t** = 2.450 against t* = 1.993); simultaneous confidence intervals (x̄ᵢ − x̄ⱼ) ± t**·SE; and the two reporting conventions, "p < .017" against a Bonferroni-adjusted p, both labeled, with a sentence on which one SPSS prints.
3. A second viz: 🎮 Contrast Builder — the group means, SDs and n from a ?g= dataset (default: the P89 frozen set) with an editable coefficient row that checks Σaᵢ = 0 live, and readouts for ψ, SE_ψ, t, df, p (one- and two-tailed) and the CI; a "pairwise" mode that lists every pair's t with the plain and Bonferroni verdicts side by side. Presets ?g= and ?c=<coefficients>. Verify one worked contrast end to end with node -e (four groups of four, control vs the mean of the rest) and cite it in the prose; t*(12) = 2.179 is the critical the CI uses there.
4. software.js one-way-anova / post-hoc-tests: the SPSS One-Way ANOVA → Contrasts subdialog (enter coefficients, the Contrast Coefficients and Contrast Tests tables, "Assume equal variances" vs "Does not assume"), and the Post Hoc subdialog's Bonferroni output (that SPSS prints adjusted p-values, not α/k); JASP's Contrasts tab. formulas.html: the contrast rows and the pairwise t.
5. Registries: glossary (planned contrast, contrast coefficient, simultaneous confidence interval), quiz +3 tagged s: "2.3", checks.js revisited, 2 new problems (a control-vs-treatments contrast with its CI; three pairwise t's with the Bonferroni decision and the APA sentence) with verification comments.

Verify: audit; prose-lint --strict; math-check (a Contrast Builder anchor driven under the shim); widget-terms, advice-terms, symbol-names (ψ is new — name it: "ψ (psi)") clean; presets; the subpath server. Rebuild FAQs and the search index; teachers.html's preset table. Tick P90 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P91 — Weeks 2–3: factorial ANOVA the lecture way

**→ Extra Powerful · Max effort**

```
StatsCapybara roadmap prompt P91 (see ROADMAP.md, Phase 18 addendum). Run node tools/audit.js first. Run alone.

§2.4's interaction-plot explorer is good and short (2,000 words). The lecture builds on it four things the lesson lacks: the two-way ANOVA table, marginal means and simple main effects computed from the cell means, the moderation vocabulary, and the reason the second factor makes the test more powerful.

1. §2.4 factorial-anova-two-way gains: factor / level / cell vocabulary and the "2 × 2", "3 × 3", "4 × 2" naming; interaction as moderation (a moderator changes the strength or direction of the other factor's effect — say the word, since §2.11 and the Methods course use it) with the reasons to study interactions (testing limits and generalization; testing a theory); crossover vs spreading interactions named beside the site's fanning/crossing; the four-panel pattern reading (which of A, B, A × B is significant from a picture); marginal means and simple main effects from the four cell means — the main effect of A compares the two A margins, the simple main effect of A at B₁ compares two cells, and the interaction is the difference of those differences; the two-way ANOVA table with df I − 1, J − 1, (I − 1)(J − 1), N − IJ, N − 1 and three F ratios sharing one MSE; the efficiency argument (a 3 × 3 with 14 per cell is 126 people where two one-way studies of the same precision need 252); the interpret-the-interaction-first rule with the sentence that a null main effect under a significant interaction does not mean "never differs".
2. The Interaction Plot Explorer gains a cell-means table beside the plot: the four cells editable (as now), the two row margins, the two column margins, the grand mean, and readouts for main effect A, main effect B, the four simple main effects and the difference of differences, so a lecturer can type any 2 × 2 into it. Presets ?cells=a1b1,a1b2,a2b1,a2b2.
3. A second viz: 🎮 Where the Variance Goes — a frozen 2 × 2 dataset (n per cell editable) analyzed two ways side by side: as a one-way ANOVA on factor A alone and as the two-way model; two stacked bars partition SST into SSA / SSB / SSAB / SSE (the one-way bar has everything but SSA inside its error), the two tables below with df, MS, F and p, and an "effect of B" slider that moves variance out of the error bar and lifts the F for A. Presets ?g= (four groups in the cell order). Verify the partition on the frozen default with node -e and assert it in math-check (VIZ.sd divides by n — use the sample helper).
4. Registries: glossary (marginal mean, simple main effect, moderator, cell), quiz +3 tagged s: "2.4", checks.js revisited, 2 new problems (a 2 × 2 cell-means table → margins, simple main effects, the interaction sign, a line-graph sketch; a two-way table with cells blanked to complete, then the reading order) with verification comments; software.js factorial: Univariate GLM's Tests of Between-Subjects Effects rows, Estimated Marginal Means, and how to get simple main effects (EMMEANS /COMPARE) — plus the JASP route. formulas.html: the two-way df and SS rows.

Verify: audit; math-check; prose-lint --strict; widget-terms (new readout labels), link-promises clean; presets; the subpath server; the cell-means table at 360px. Rebuild FAQs and the search index; teachers.html's preset table. Tick P91 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P92 — Weeks 3–4: repeated measures, the table and the mixed design

**→ Extra Powerful · Max effort**

```
StatsCapybara roadmap prompt P92 (see ROADMAP.md, Phase 18 addendum). Run node tools/audit.js first. Run alone.

§2.5's subject-centering viz shows why repeated measures work and its sphericity section is strong. What the lecture adds and the lesson lacks: the RM table with the subjects' variance removed, the identity that explains the paired advantage, mixed between × within designs, and the design costs (order, carryover, counterbalancing — zero hits on the site).

1. §2.5 repeated-measures-anova gains: RM-ANOVA extends the paired t the way one-way extends the independent t (conditions, not groups); the difference-score view for two conditions (D = X₂ − X₁, H₀: μ_D = 0) and the identity Var(X − Y) = Var(X) + Var(Y) − 2·Cov(X, Y), with Cov = r·SD(X)·SD(Y), worked on a five-person set of your own in the lecture's shape (two columns whose SDs are around 25 while their differences have SD around 2; verify, and print the paired t against the independent t on the same numbers); the partition SS_total = SS_conditions + SS_within, SS_within = SS_subjects + SS_error, with df k − 1, n − 1, (n − 1)(k − 1), N − 1, and F = MS_conditions/MS_error; a worked table on a frozen n × k dataset, and the bookkeeping trick an exam uses — from "F(3, 84)" alone, recover 29 participants; a mixed-design section (one between-subjects factor and one within-subjects factor; the two output tables SPSS prints; the df table for between, within, interaction and their two error terms; interpret the interaction first, then each main effect "averaging over" the other); order effects, carryover, practice and fatigue, counterbalancing (what it does and does not fix); the advantages and disadvantages list in the lecture's shape; sphericity stays where it is.
2. The existing viz gains a partition readout beneath the plot: the same data analyzed as a one-way ANOVA (people ignored) and as RM, two stacked bars (SS_conditions / SS_subjects / SS_error, with the one-way bar folding subjects into error), the two tables with df, F and p. The "How different people are" slider then visibly moves variance from the error bar into the subjects bar and lifts F. Presets ?spread= ?effect= ?view=. VIZ.sd divides by n — use the sample helper; math-check asserts the partition on the frozen default.
3. software.js repeated-measures-anova: reading Tests of Within-Subjects Effects (Sphericity Assumed / Greenhouse-Geisser / Huynh-Feldt / Lower-bound rows, the Error(factor) line), Mauchly's table, and for a mixed design which table holds the between factor; the JASP route. formulas.html: the RM SS and df rows, the covariance identity.
4. Registries: glossary (within-subjects factor, between-subjects factor, mixed design, counterbalancing, carryover effect — check what the Methods course already defines and link rather than duplicate), quiz +3 tagged s: "2.5", checks.js revisited, 2 new problems (complete an RM table from SS_conditions, SS_subjects, SS_total and n, k; read a mixed-design output — which effect is which, how many participants, what to interpret first) with verification comments.

Verify: audit; math-check; prose-lint --strict; widget-terms, link-promises (the Methods links) clean; presets; the subpath server. Rebuild FAQs and the search index; teachers.html's preset table. Tick P92 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P93 — Weeks 4–7: regression inference and multiple regression the lecture way

**→ Extra Powerful · Max effort**

```
StatsCapybara roadmap prompt P93 (see ROADMAP.md, Phase 18 addendum). Run node tools/audit.js first. Run alone.

§2.7 already carries the model, s on n − 2, SE(b₁), the t and CI and the regression ANOVA table. The lecture's multiple-regression half is where the site is thin: omitted-variable bias has a formula and a sign rule in the course and "can bias" on the site; n − p − 1 has zero hits sitewide.

1. §2.7 inference-for-regression: define covariance and write b₁ = cov(x, y)/var(x) beside §1.18's r·sᵧ/sₓ (the same number twice; show it); SE(b₀) beside SE(b₁) with the same intuition (more spread in x and less scatter around the line both help); the conditions in the course's wording — independence, linearity, equal variance (homoscedasticity), normality, and no relation between the predictors and the error term (exogeneity), with the sentence that under these b is the best linear unbiased estimate (BLUE) and a link forward to §2.9 for what breaks when exogeneity fails; reading an SPSS Coefficients table (B, Std. Error, t = B/SE, Sig., the CI columns) and the Model Summary / ANOVA tables above it, tied to the lesson's own numbers.
2. §2.9 multiple-regression gains a section on omitted-variable bias with the arithmetic: if the truth is y = β₀ + β₁x₁ + β₂x₂ + ε and you fit y on x₁ alone, E[b₁] = β₁ + β₂·δ where δ is the slope of x₂ on x₁; the bias is the product of "how much the omitted variable matters" and "how related it is to the included one", so its SIGN follows a 2 × 2 table (positive × positive → overestimate, and the other three cells); a worked example of your own in the wage-and-motivation shape, ending in the honest sentence "the effect is at most …". The Power of "Controlling For" viz gains readouts for β₂, δ, the bias β₂·δ and the naive-minus-controlled slope, plus a sign-quadrant indicator; presets ?conf= exists — add ?b2= and ?delta= if the viz's parameters allow. Also in §2.9: the df n − p − 1 named everywhere a t or an s appears; the overall F(p, n − p − 1) test and what its rejection does and does not say; the multiple correlation R = √R² as the correlation between y and ŷ; the adjusted-R² formula written out, 1 − (1 − R²)(n − 1)/(n − p − 1), with the penalty visible; interpreting a model with one continuous and one dummy predictor ("holding height constant, … ; holding sex constant, …").
3. §2.12 multicollinearity-and-variable-selection: perfect collinearity (software drops a predictor) vs the practical kind (pairwise r beyond ±.8 or ±.9; a predictor well predicted by the others), VIF = 1/(1 − R²ⱼ) with the "regress each predictor on the rest" recipe and the textbook's VIF > 10 line; and a section that labels both dialects on model selection — theory-first specification, and general-to-specific (start general, remove by t, adjusted R² and AIC) as a procedure that some courses teach, with the site's existing stepwise warning kept beside it as its cost (a-theoretical, ignores omitted-variable bias, multiplies tests). §2.10: the ANOVA-as-regression demonstration done in full on the site's study-methods.csv (three groups → two dummies; the regression ANOVA table equals the one-way table; the coefficient p-values are the pairwise comparisons against the reference; the k-th dummy is perfectly collinear and software drops it).
4. formulas.html: covariance, b₁ = cov/var, SE(b₀), the multiple-regression df and F, R, adjusted R², VIF, omitted-variable bias. Registries: glossary (covariance, omitted-variable bias, exogeneity, BLUE, multiple correlation, general-to-specific — check what exists), quiz +3 across s: "2.7", "2.9", "2.12", checks.js revisited, 3 new problems (slope t and CI from B and SE at n − 2; adjusted R² and the overall F from R², n, p; the sign of an omitted-variable bias from a story) with verification comments.

Verify: audit (CHECK 9 will read the new APA sentences); prose-lint --strict; widget-terms (new readouts), advice-terms, untaught-names (BLUE is an acronym — explain it in prose), symbol-names (δ is new — name it), link-promises clean; numbers recomputed on the shipped CSV; presets; the subpath server. Rebuild FAQs and the search index; teachers.html's preset table. Tick P93 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P94 — Exam prep: the Stats 2 exam-style set, the numbering fix, the formula sheet

**→ Powerful · Extra**

```
StatsCapybara roadmap prompt P94 (see ROADMAP.md, Phase 18 addendum). Requires P89–P92 (their tables and formula rows). Run node tools/audit.js first. Edits problems.html, formulas.html, tools/audit.js: run alone.

Three things for a paper exam on ANOVA and regression, and one repair the P86 tick left for its own session.

1. The repair first. problems.html's visible problem numbers duplicate across sets since P79 and P82 appended to the Stats 1 set (the Stats 2 set still starts at 13), and lesson prose cites problems by number ("Problem 15 of the practice problems …"). Renumber the badges continuously across all sets in page order, then update EVERY prose reference: grep for "Problem \d+" and "Problems \d+" across lessons, guides, FAQ answers in tools/faq_data.py (then rerun inject-faqs.py), teachers.html and checks/software strings, and repoint each to the fragment it names. Then close the class: add a CHECK to tools/audit.js that pairs any "Problem N" text within 60 characters of a problems.html#p… link with the badge rendered at that fragment and errors on a mismatch — the same shape as the section-label check the restructure added. Prove it by reintroducing one stale number.
2. A second Stats 2 set, "Exam-style · A midterm on §2.1–§2.5", 8 problems in the shapes an ANOVA exam asks: (1) complete a one-way ANOVA table with three cells blanked, then η² and sₚ; (2) from group means, SDs and n to F, the verdict, and F = t² as a check on a two-group version; (3) a planned contrast with its SE, t, one-tailed p and CI; (4) three pairwise t's with the Bonferroni t** and the APA sentence; (5) a 2 × 2 cell-means table → margins, simple main effects, the interaction's sign, then complete the two-way table; (6) an RM table from the partition with n and k recovered from the df; (7) Mann–Whitney U by hand with the table decision and the exact p beside it; (8) Spearman's ρₛ by hand with its table. Every number verified with node -e (VIZ for F and t; exact enumeration for U and ρₛ, in the comment), difficulty chips, .pb-ref lines to §2.1–§2.5 and to the tools that check them (the ANOVA-table builder with ?g=, tables.html). The site's own numbers and stories.
3. formulas.html read as one sheet: the Stats 2 rows P89–P93 added, in the order the course meets them, with the notation consistent across rows (I groups, N total, k conditions, n people, p predictors — pick one convention per family and state it in the table caption); add the logistic rows the sheet lacks if any (the logit, the odds ratio from b, the Wald z), since chapter 14 is examined and no slides were provided. The printable sheet must still fit its page count — verify with the P30 CSSOM flip.
4. teachers.html: the "Set a mock exam" example gains a Stats 2 line (quiz.html?exam=stats-2&from=2.1&to=2.5&n=20) beside the Stats 1 one.

Verify: audit (the new CHECK live, plus fragment ids); prose-lint --strict; the problems page's search-index cap (PAGE_MAX_CHARS — the P86 tick found it overrun once; check the rebuilt index finds the last problem's text); the subpath server. Rebuild FAQs and the search index. Tick P94 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P95 — The research-project guide: one question, an ANOVA and a regression

**→ Extra Powerful · Ultra**

```
StatsCapybara roadmap prompt P95 (see ROADMAP.md, Phase 18 addendum). Run node tools/audit.js first. Run alone.

Intermediate courses end in a group research project of one shape: collect your own data on a question, analyze it as a one-way ANOVA of at least three conditions AND as a multiple regression of the same relation with control variables, in SPSS, and write it up as a paper. The site has every stop and no page that walks that exact route (the capstone guide is a 2 × 2 factorial). Build guides/anova-and-regression-project/ on the P34/P72 pattern.

1. The route, each step on a shipped dataset so every number is true: (1) a testable question whose predictor has three or more conditions and whose outcome is continuous, and the control variables a reviewer will ask about (link methods/from-question-to-hypothesis and methods/variables-and-operationalization); (2) collecting the data — a survey or a small experiment, with the design choices that decide the analysis (between vs within, the sample the ANOVA needs from the power calculator, the survey-design lesson); (3) the codebook and the cleaning workflow (data/codebooks-and-documentation, data/data-cleaning-workflow); (4) univariate checks — distributions, normality, outliers, per condition (descriptives.html, data/outliers-in-practice); (5) the one-way ANOVA with its assumption checks, the ANOVA table read row by row, a planned contrast or the post-hoc comparisons, η²; (6) the same question as a regression: dummies for the conditions, the control variables added, the coefficients read as "holding … constant", the omitted-variable argument for why the controls are there, VIF, the residual plot; (7) the two analyses reconciled (the ANOVA table equals the dummies-only regression; what the controls changed and why); (8) the paper — IMRaD, the APA sentences from apa.html, the figure (a slopegraph or means-with-CIs), limitations; (9) a checklist a supervisor could grade against (question, design, data quality, assumptions, the two analyses, interpretation, limitations, reporting).
2. The dataset: extend or add one so the route is honest — a three-condition study with a continuous outcome and two plausible covariates (study-methods.csv has the three groups; if it lacks covariates, ship a new CSV in the datasets library with its card and verification comment, generated with a documented seed and a clear "simulated for teaching" label). Every mock SPSS table in the guide is computed from that CSV with node -e and checked; the ANOVA and the dummies-only regression must agree to the printed decimals.
3. Voice and honesty: VOICE.md throughout; the guide teaches the method on the site's data and says once, plainly, that a course's rules on AI help with a student's own dataset are the student's to follow, then never does a student's analysis for them. No institution named.
4. The seven registrations (TOOLBOX group "read", SEARCH_PAGES, QUIPS, build-search-index.py, sitemap.xml, audit.js GUIDES, prose-lint.js GUIDES) plus the "Keep going" links from §2.2, §2.9, §2.10 and the capstone guide, and a card on guides/index.html. Head: og:type article, brand og.png, [Article, BreadcrumbList] JSON-LD with the Guides level pointing at /guides/.

Verify: audit (CHECK 9 reads the APA sentences); prose-lint --strict (a new ~2,500-word page — mind rule 1's two clauses); worked-examples.js (every η² beside its F); link-promises and destination-promises on the new links; the guide is indexed whole (GUIDE_MAX_CHARS); the subpath server; light + dark + 360px. Tick P95 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

### P96 — The instructor surface for Stats 2

**→ Powerful · Extra**

```
StatsCapybara roadmap prompt P96 (see ROADMAP.md, Phase 18 addendum). Run AFTER P88–P95: it documents their presets. Run node tools/audit.js first.

The instructor surface for an intermediate block, beside the Stats 1 one P87 built.

1. teachers.html: an eight-week block map for a stand-alone Stats 2 beside the seven-week Stats 1 map (same .sem-map / .bm-map / .hscroll pattern, same print path), in the order an intermediate course teaches: non-parametric tests and one-way ANOVA; contrasts and multiple comparisons; factorial ANOVA; repeated measures and mixed designs; a midterm week with inference for regression; multiple regression; multiple regression continued and model selection; logistic regression and non-linearities; then the final and a project week. Each cell links the lessons with the preset a lecturer would put on the slide (?g= on §2.2, §2.3's builder and the Where-the-variance-goes vizzes, ?cells= on §2.4, ?spread=&effect= on §2.5, ?tab=&g= on §2.1), the tools (the ANOVA-table builder, tables.html's U/W/ρₛ tables, apa.html's ANOVA and regression presets, the project guide) and the problem sets. Prints on one page (the P30 CSSOM flip, both paper sizes).
2. The "Set a mock exam" section shows both courses' ranges; the "A ready-made semester project" section links the new project guide as the intermediate option beside the capstone.
3. Completeness sweep: the preset table lists every lesson that calls SC.preset after P88–P95 (rerun P87's node check; paste the result under the tick), ?g= is documented beside ?data= and ?data=x:y with its comma/semicolon rule, CLAUDE.md's preset passage and "Adding a lesson" checklist mention it, and the fourteen-week semester map's Stats 2 rows point at the new presets where one fits.

Verify: audit; prose-lint --strict; light + dark + 360px; both block maps print on one page each; the subpath server. Tick P96 in ROADMAP.md, then commit and push to main (CLAUDE.md's "Finishing a session" rule).
```

---

*End of prompt library. Phases 17 and 18 (P79–P96, Sep 2026) are the first phases written from the outside in: a real course's slides and syllabus read against the site, and the site brought to speak the textbook dialect its students are examined in. After P72 the site is built: 9 courses with landing pages, 97 interactive lessons, an exam mode, a 43-problem worked-problems library, 20+ tools, 5 guides, 3 posters, print/offline/a11y polish, instructor embeds that carry their configuration, math under regression test, honest analytics honestly disclosed, and forgiving search. From there the roadmap IS the loops — P37 (waiting on a Search Console export), P38 quarterly (its next run should sweep the P61–P68 surfaces), P39 refresh — plus the human-only checklist, which is now the growth engine: distribution, not construction. Phases 12–15 (P49–P72) were review-driven punch-lists; anything proposed beyond them should have to argue its way past "the site doesn't need it". Phase 16 (P73–P78) argued its way past it with a measurement: the de-AI job was budget-met, not finished — a paydown and a provenance fix, not a feature.*
