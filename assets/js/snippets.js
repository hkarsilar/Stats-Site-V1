...
window.SNIPPETS = {
  ...
  "repeated-measures-anova": {
    r: 'library(afex)   # install.packages("afex")\n# long format: one row per subject x condition\naov_ez(id = "subj", dv = "rt", within = "condition", data = df)\n# afex applies the Greenhouse-Geisser correction for you\n\n# to see the epsilon itself, from wide data (one row per subject):\nfit <- lm(cbind(t1, t2, t3) ~ 1, data = wide)\nidata <- data.frame(cond = factor(1:3))\nsummary(car::Anova(fit, idata = idata, idesign = ~cond), multivariate = FALSE)\n# prints Mauchly W, the GG and HF epsilons, and both corrected p-values',
  ...
  "sampling-methods": {
    r: 'set.seed(1)\n# simple random sample of 80 rows\nsrs <- df[sample(nrow(df), 80), ]\n\n# stratified: ~20 per stratum, sampled within each group\nlibrary(dplyr)\nstrat <- df %>% group_by(stratum) %>% slice_sample(n = 20) %>% ungroup()\n\n# cluster: pick 4 whole schools and take everyone inside them\nclus <- df %>% filter(school %in% sample(unique(df$school), 4))\n\n# what the clustering costs: design effect and effective sample size\nicc  <- 0.10                                  # between-cluster share of the variance\ndeff <- 1 + (mean(table(clus$school)) - 1) * icc\nc(rows = nrow(clus), deff = deff, n_eff = nrow(clus) / deff)',
    py: 'import pandas as pd\n# simple random sample of 80 rows\nsrs = df.sample(n=80, random_state=1)\n\n# stratified: ~20 per stratum, sampled within each group\nstrat = df.groupby("stratum").sample(n=20, random_state=1)\n\n# cluster: pick 4 whole schools and take everyone inside them\npicks = pd.Series(df["school"].unique()).sample(4, random_state=1)\nclus = df[df["school"].isin(picks)]\n\n# what the clustering costs: design effect and effective sample size\nicc = 0.10                                    # between-cluster share of the variance\ndeff = 1 + (clus.groupby("school").size().mean() - 1) * icc\nprint(len(clus), deff, len(clus) / deff)'
  },
  ...
  "merging-datasets": {
    r: 'library(dplyr)\n# left join: keep every participant, attach sessions where the key matches\nmerged <- left_join(participants, sessions, by = "id")\ninner  <- inner_join(participants, sessions, by = "id")   # complete cases only\nfull   <- full_join(participants, sessions, by = "id")    # keep everyone\n# state the shape you expect and let the join refuse to run when it is wrong.\n# a one-to-many join is SILENT by default -- only many-to-many warns\nleft_join(participants, sessions, by = "id", relationship = "one-to-one")\n# who failed to match? anti_join names them, in both directions\nanti_join(participants, sessions, by = "id")   # participants with no session\nanti_join(sessions, participants, by = "id")   # sessions with no participant\nnrow(participants); nrow(merged)   # a left join should not lose rows',
    py: 'import pandas as pd\n# left merge: keep every participant, attach sessions where the key matches\nmerged = participants.merge(sessions, on="id", how="left")\ninner  = participants.merge(sessions, on="id", how="inner")   # complete cases\nouter  = participants.merge(sessions, on="id", how="outer")    # keep everyone\n# state the shape you expect; pandas raises MergeError when the data disagree\nparticipants.merge(sessions, on="id", how="left", validate="1:1")\n# who failed to match? indicator= labels every row left_only / right_only / both\naudit = participants.merge(sessions, on="id", how="outer", indicator=True)\nprint(audit[audit["_merge"] != "both"])\nprint(len(participants), "->", len(merged))   # a left merge should not lose rows'
  },
  "reproducible-workflows": {
    r: 'set.seed(2026)      # FIRST line: everything random below is now repeatable\n\nraw   <- read.csv("raw/survey.csv")            # raw/ is read-only, always\nclean <- subset(raw, age >= 18 & !is.na(score))\nwrite.csv(clean, "clean/survey_clean.csv", row.names = FALSE)\n\nfit <- lm(score ~ age, data = clean)\nsaveRDS(fit, "output/model.rds")               # output/ is regenerated, never hand-made\n\n# the seed alone is not enough -- record what ran it.  sessionInfo() prints the R\n# version, the OS, and the version of every package you loaded\nwriteLines(capture.output(sessionInfo()), "output/session_info.txt")',
    py: 'import numpy as np, pandas as pd, subprocess\nrng = np.random.default_rng(2026)   # FIRST line: pass rng to every random step below\n\nraw   = pd.read_csv("raw/survey.csv")           # raw/ is read-only, always\nclean = raw[(raw["age"] >= 18) & raw["score"].notna()]\nclean.to_csv("clean/survey_clean.csv", index=False)\n\n# the seed alone is not enough -- record what ran it\nwith open("output/requirements.txt", "w") as f:\n    f.write(subprocess.run(["pip", "freeze"], capture_output=True, text=True).stdout)'
  },
